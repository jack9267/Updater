...
FILE* g_pInstallerLog;

static void Print(const char *s)
{
  if (g_pInstallerLog != nullptr)
    fputs(s, g_pInstallerLog);
}
...
extern CRITICAL_SECTION g_ProgressCriticalSection;
extern bool g_bUpdateProgress;
extern int g_ProgressTotal;
extern int g_ProgressCurrent;
extern TCHAR g_szCurrentFile[256];

int MY_CDECL Main(HWND hMessageLabel, HWND hProgressBar)
{
  ISzAlloc allocImp;
  ...
  #else
  if (InFile_OpenW(&archiveStream.file, L"update.7z"))
  #endif
  {
    PrintError("Can not open update file");
    return 1;
  }
  ...
  if (res == SZ_OK)
  {
    ...
    if (res == SZ_OK)
    {
      ...
      */
      ...
      PostMessage(hProgressBar,PBM_SETRANGE,0,MAKELPARAM(0,db.NumFiles));
	  ...
	  for (i = 0; i < db.NumFiles; i++)
      {
		...
		EnterCriticalSection(&g_ProgressCriticalSection);
		g_bUpdateProgress = true;
		g_ProgressTotal = db.NumFiles;
		g_ProgressCurrent = i;
		_stprintf_s(g_szCurrentFile, _T("Extracting %s ..."), temp);
